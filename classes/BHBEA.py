...
try:
    import BHBEAAPI as api
except:
    from classes import BHBEAAPI as api

def set_user_id(id):
    
    api.Request._user_id = '3A5B39F9-C5E8-4DA0-BD4F-F9A1EAC569A4'
...
def _get_NIPA(ResultFormat, TableIDs,  **params):
    """Combines data on multiple KeyCodes into a single Pandas DataFrame."""
    dfs = []
...
def _get_NIUnderlyingDetail(ResultFormat, TableIDs, **params):
    """Combines data on multiple KeyCodes into a single Pandas DataFrame."""
    dfs = []
    for TableID in TableIDs:
        tmp_request = api.NIUnderlyingDetailRequest(ResultFormat=ResultFormat,
                                              TableID=TableID,
                                              **params)
